...
*/
//创建信号量集
//一:从空标志组链表中取下一个标志组,同时给成员OSFlagType和OSFlagFlags赋值
//二:令指向等待任务链表的指针OSFlagWaitList为空指针.
//flags:信号的初始化值
//*perr:错误信息
                            ...
                            INT8U    *perr)
{
    ...
    if (OSIntNesting > 0u) {                        /* 中断中不能调用OSFlagCreate()  See if called from ISR ...                      */
        *perr = OS_ERR_CREATE_ISR;                  /* ... can't CREATE from an ISR                    */
        return ((OS_FLAG_GRP *)0);
    }
    OS_ENTER_CRITICAL();
    pgrp = OSFlagFreeList;                          /* 获取自由标志组 Get next free event flag                        */
    if (pgrp != (OS_FLAG_GRP *)0) {                 /* 判断获取到的标志组是否可用 See if we have event flag groups available      */
                                                    /* Adjust free list                                */
        OSFlagFreeList       = (OS_FLAG_GRP *)OSFlagFreeList->OSFlagWaitList;  //OSFlagFreeList指向下一个空闲标志组
        pgrp->OSFlagType     = OS_EVENT_TYPE_FLAG;  /* OSFlagType标志为OS_EVENT_TYPE_FLAG Set to event flag group type                    */
        pgrp->OSFlagFlags    = flags;               /* 设置OSFlagFlags的值 Set to desired initial value                    */
        pgrp->OSFlagWaitList = (void *)0;           /* 清除等待任务列表 Clear list of tasks waiting on flags            */
...
*********************************************************************************************************
*/
//请求信号量集,flags是用来给等待任务链表节点成员OSFlagNodeFlags赋值的
//*pgrp:所请求的信号量集指针
//flags:滤波器
//wait_type:逻辑运算类型
//timeout:等待时限
                      ...
                      INT8U        *perr)
{
...
#endif
    if (OSIntNesting > 0u) {                               /* 中断中不能调用此函数 See if called from ISR ...               */
    ...
    if (pgrp->OSFlagType != OS_EVENT_TYPE_FLAG) {          /* 验证事件控制块的类型 Validate event block type                */
        *perr = OS_ERR_EVENT_TYPE;
        return ((OS_FLAGS)0);
    }
    result = (INT8U)(wait_type & OS_FLAG_CONSUME);
...
/*$PAGE*/
    OS_ENTER_CRITICAL();
    switch (wait_type) {                                   //根据不同的逻辑运算方式来计算
        case OS_FLAG_WAIT_SET_ALL:                         /* 判断是否要求所有的信号有效(全1)See if all required flags are set        */
             flags_rdy = (OS_FLAGS)(pgrp->OSFlagFlags & flags);   /* 提取出我们想要的信号量 Extract only the bits we want     */
             if (flags_rdy == flags) {                     /* 必须和我们期望的所有信号量匹配 Must match ALL the bits that we want     */
                 if (consume == OS_TRUE) {                 /* See if we need to consume the flags      */
                     pgrp->OSFlagFlags &= (OS_FLAGS)~flags_rdy;   /* Clear ONLY the flags we wanted    */
                 }
        ...
        case OS_FLAG_WAIT_SET_ANY:
             flags_rdy = (OS_FLAGS)(pgrp->OSFlagFlags & flags);    /* 提取我们想要的信号量 Extract only the bits we want    */
             if (flags_rdy != (OS_FLAGS)0) {               /* 判断是否至少有一个信号量为1 See if any flag set                      */
                 if (consume == OS_TRUE) {                 /* See if we need to consume the flags      */
...
/*$PAGE*/
    OS_Sched();                                            /* 发起一个任务 Find next HPT ready to run               */
    OS_ENTER_CRITICAL();
